...
export const ipPortSchema = z.string()
  .trim()
  .superRefine((value, ctx) => {
    ...
    const ipValidationResult = z.string().ip({
      message: `Invalid IP address.`
    }).safeParse(ipPart);

    if (!ipValidationResult.success) {
      ipValidationResult.error.issues.forEach(issue => ctx.addIssue({
        ...issue,
        path: [...ctx.path, "ip"]
      }));
    }
...
export const listenerSchema = z.strictObject({
  ...
  agentId: z.coerce.number({ "message": "invalid agent id" }).int({ "message": "invalid agent id" })
});
