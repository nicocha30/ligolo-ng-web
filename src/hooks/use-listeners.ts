...
export default function useListeners(): {
  mutate: <MutationData = Listener[]>(
    data?:
      | Promise<Listener[] | undefined>
      | MutatorCallback<Listener[]>
      | Listener[],
    opts?: boolean | MutatorOptions<Listener[], MutationData>,
  ) => Promise<Listener[] | MutationData | undefined>;
  loading: boolean;
  listeners: Listener[];
} {
  const auth = useAuth();
  ...
  const { data, mutate, isLoading } = useSWR<LigoloListeners>(
    `${auth?.api}/listeners`,
